...
export class JpaMasterPolicyComponent implements OnInit {
  ...
  constructor(private el: ElementRef, public dialog: MatDialog, private router: Router,
    private fb: FormBuilder, private workflowService: DoiService, private utilityService: JpaUtilityService, private datepipe: DatePipe) { }
  ...
  ngOnInit() {
    ...
    this.getMasterPolicyListingData(new JpaMasterPolicyBase());
  ...
  public onSearchClicked() {
    let payload: JpaMasterPolicyBase = new JpaMasterPolicyBase();
    payload.policyTypeId = this.jpaClaimEntry.controls.policyTypeL.value;
    payload.policyId = this.jpaClaimEntry.controls.ploicyNo.value;
    payload.beneficiaryNum = this.jpaClaimEntry.controls.numberOfBenificiariesL.value;
    payload.schemeName = this.jpaClaimEntry.controls.schemeTypeL.value;
    payload.policyStatusId = this.jpaClaimEntry.controls.status.value;

    this.getMasterPolicyListingData(payload);
  }
  validateForm() {
  ...
  public addRecord() {
    ...
    payload.policyStartDate = this.datepipe.transform(new Date(this.jpaClaimEntry.controls.startDate.value), 'yyyy-MM-dd');
    payload.policyEndDate = this.datepipe.transform(new Date(this.jpaClaimEntry.controls.endDate.value), 'yyyy-MM-dd');
  ...
  public saveRecord() {
    if (this.cacheRecords.length > 0) {
      this.cacheRecords.forEach((x: any) => {
        this.addUpdatePolicyData(x);
      });
  ...
  private getMasterPolicyListingData(param: JpaMasterPolicyBase) {
