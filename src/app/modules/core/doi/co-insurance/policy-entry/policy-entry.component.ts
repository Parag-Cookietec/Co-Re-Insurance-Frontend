...
export class PolicyEntryComponent implements OnInit {
  ...
  lkPoOffUserId: any;
  wfRoleId: any;
  constructor(private fb: FormBuilder, private router: Router, private dialog: MatDialog, private workflowService: DoiService,
  ...
  selectDistrict() {
    ...
    if (district !== '' && district != null) {
      this.workflowService.getRequestWithNumberPathVar(APIConst.GET_DOI_TALUKA_LIST, district).subscribe(
        (resp: any) => {
          if (resp && resp.result && resp.status === 200) {
            this.talukaNameList = resp.result;
          }
        }
      );
  ...
  goToNextTab() {
    console.log(this.policyEntryForm.value);
    this.selectedIndex++;
  }
