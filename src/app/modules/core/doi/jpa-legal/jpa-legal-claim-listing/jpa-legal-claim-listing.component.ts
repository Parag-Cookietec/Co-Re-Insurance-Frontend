...
export class JpaLegalClaimListingComponent implements OnInit {
  ...
  fetchAllLegalEntries() {
    ...
    this.workflowService.getData(passData,APIConst.DOI_JPA_LEGAL_CLAIM_LISTING)
    .subscribe((data:any) => {
  ...
  dateFormatter(date:string, pattern: string) {
   ...
   searchDisplay() {
    ...
    let cnt = 0;
    if(this.jpaLegalClaim.value.district !== '' && this.jpaLegalClaim.value.district !==undefined) {
      
      opt[cnt++]= {'key':'districtId', 'value':this.jpaLegalClaim.value.district };
    }
    if(this.jpaLegalClaim.value.month !== '' && this.jpaLegalClaim.value.month !==undefined) {
    
      opt[cnt++]= {'key':'claimMonthId', 'value':this.jpaLegalClaim.value.month };
    }
    if(this.jpaLegalClaim.value.year !== '' && this.jpaLegalClaim.value.year !==undefined) {
   
      opt[cnt++]= {'key':'claimYearId', 'value':this.jpaLegalClaim.value.year };
    }
    if(this.jpaLegalClaim.value.disabledDied !== '' && this.jpaLegalClaim.value.disabledDied !==undefined) {
      
     // opt[cnt++]= {'key':'personTypeId', 'value':this.jpaLegalClaim.value.disabledDied };
    }
    if(this.jpaLegalClaim.value.fromDate !== '' && this.jpaLegalClaim.value.fromDate !==undefined) {
     
      let date = this.dateFormatter(this.jpaLegalClaim.value.fromDate,"dd/MM/yyyy");
      opt[cnt++]= {'key':'createdDate', 'value':date, 'operation':'>='};
    }
    if(this.jpaLegalClaim.value.createdDate !== '' && this.jpaLegalClaim.value.createdDate !==undefined) {
    
      let date = this.dateFormatter(this.jpaLegalClaim.value.toDate,"dd/MM/yyyy");
      opt[cnt++]= {'key':'createdDate', 'value':date,'operation':'<=' };
    }
